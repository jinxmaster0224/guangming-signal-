...
def generate_charts(df_interval, df_raw, intersection_name, output_dir, interval):
    ...
    for col, title, ylabel in metrics:
        ...
        if SHOW_OVERLAY_ON_COMBINED_CHART:
            add_custom_periods_overlay(ax_comb, CUSTOM_PERIODS, right_ylabel='周期时间 (秒)')
    ...
    if 'turn_dir_no' in df_raw.columns:
        ...
        if not df_turn.empty:
            ...
            ax_turn.set_title('各进口道转向平均流量', fontsize=14, fontweight='bold')
...
def analyze_traffic_data(dir_csv_path, info_csv_path, date_ranges, offset_degree=0.0):
    ...
    # === 新增模块：5分钟历史窗口求 pass_flow 平均值 ===
    print("  -> [进度] 正在计算车流量(pass_flow)的 5 分钟历史窗口平均值...")
    df = df.set_index('create_time').sort_index()
    # 尽可能按照最细颗粒度（frid 或 方向+转向）分组计算历史平均
    if 'frid' in df.columns:
        group_keys = ['frid']
    elif 'turn_dir_no' in df.columns:
        group_keys = ['main_direction', 'turn_dir_no']
    else:
        group_keys = ['main_direction']
        
    df['pass_flow'] = df.groupby(group_keys)['pass_flow'].transform(
        lambda x: x.rolling('5min', min_periods=1).mean()
    )
    df = df.reset_index()
    # ===================================================

    # === 修复点：清洗 0 值，防止拉低不停车通过速度的平均数 ===
    ...
    for interval in intervals:
        ...
        if interval == 1:
            ...
            numeric_cols = [c for c in ['queue_len_max', 'queue_len_avg', 'pass_flow', 'stop_time', 'stop_times', 'pass_speed', 'no_stop_pass_speed', 'delay_index'] if c in df.columns]
