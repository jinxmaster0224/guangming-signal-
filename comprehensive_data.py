...
def run_bottleneck_diagnosis_on_slice(df_dir, inter_name):
    # 此时送进来的 df_dir 已经是纯正的 5分钟切片数据了，不再需要二次抽样！
...
def analyze_multiple_files(file_paths, date_ranges, inter_config):
    ...
    for file_path in file_paths:
        ...
        # 1. 在彻底清洗过的 5 分钟代表数据集里提取路口真正的物理极限
        ...
        # 2. 回归正统数学逻辑：严格按照每一个 5 分钟切片点 (create_time) 去算同时间的总流量与占比
        ...
        export_cols = ['路口名称', '进口道方向', '转向', 'create_time', '延误指数', 'pass_flow', '历史最大分均流量', '流量占比(%)']
        for c in export_cols:
            if c not in df_export.columns: df_export[c] = None
        global_granular_data.append(df_export[export_cols])
