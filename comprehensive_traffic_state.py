...
def analyze_multiple_files(file_paths, output_dir):
    date_ranges = [(pd.Timestamp(f'{TARGET_DATE} 00:00:00'), pd.Timestamp(f'{TARGET_DATE} 23:59:59'))]
...
def run_period_analysis(df_path, time_periods, output_dir, current_plan, day_type):
...
if __name__ == "__main__":
    ...
    if not csv_files:
        ...
        # Phase 1: 数据清洗与特征提取
        processed_data_path = analyze_multiple_files(csv_files, script_dir)
        
        if processed_data_path:
            generated_files.append(("底层聚合清洗数据明细表", "干线全流向_延误与流量占比明细表.csv", processed_data_path))
            
            # Phase 2: Fisher时段划分提取
            dynamic_periods, periods_path = run_tod_partition(csv_files, script_dir)
            
            if dynamic_periods:
                generated_files.append(("Fisher肘部时段划分结果", "时段划分结果表.csv", periods_path))
                
                # Phase 3-A: 工作日周期绿信比推演
                wd_name, wd_path = run_period_analysis(
                    processed_data_path, dynamic_periods, script_dir, 
                    current_plan=CURRENT_SIGNAL_PLAN_WEEKDAY, day_type="工作日"
                )
                generated_files.append(("工作日信控推演方案结果", wd_name, wd_path))
                
                # Phase 3-B: 非工作日周期绿信比推演
                we_name, we_path = run_period_analysis(
                    processed_data_path, dynamic_periods, script_dir, 
                    current_plan=CURRENT_SIGNAL_PLAN_WEEKEND, day_type="非工作日"
                )
                ...
                print("=" * 80)
                print(f"本次运行共生成 {len(generated_files)} 个报表文件，均已成功保存至目录：\n👉 {script_dir}\n")
