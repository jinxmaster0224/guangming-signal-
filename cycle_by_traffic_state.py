...
STATE_OPTIMIZATION_WEIGHTS = {
    ...
    '高峰流量': {
        'delay_weight': 0.7,        # 修改为 70% 延误指标
        'throughput_weight': 0.0,   # 取消通行能力指标
        'queue_weight': 0.3         # 保持 30% 排队溢出风险控制
    }
}
...
def optimize_cycle_peak(phase_data, L_total, bounds):
    ...
    for C in range(bounds['min'], bounds['max'] + 1):
        ...
        # 物理限制：必须保证周期够大，足以给所有相位分配至少 20s 绿灯
        if G <= total_base_g:
            continue
        ...
        # 先给四个方向各 20s 的安全绿灯保障，然后根据饱和度要求（y_val占比）分配剩余时间
