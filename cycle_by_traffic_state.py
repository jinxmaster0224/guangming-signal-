...
STATE_OPTIMIZATION_WEIGHTS = {
    '极低流量': {
        'delay_weight': 0.3,      
        'cycle_stability_weight': 0.5,  
        'min_green_weight': 0.2   
    },
    '平峰流量': {
        'delay_weight': 0.4,      
        'cycle_stability_weight': 0.2,
...
CURRENT_SIGNAL_PLAN = {
    '现行周期方案': [
        {'start': '00:00', 'end': '06:00', 'scheme': '方案10', 'cycle': 99},
        {'start': '06:00', 'end': '07:30', 'scheme': '方案2', 'cycle': 114},
        {'start': '07:30', 'end': '09:00', 'scheme': '方案24', 'cycle': 167},
        {'start': '09:00', 'end': '12:00', 'scheme': '方案26', 'cycle': 152},
        {'start': '12:00', 'end': '14:00', 'scheme': '方案3', 'cycle': 150},
        {'start': '14:00', 'end': '16:00', 'scheme': '方案4', 'cycle': 152},
        {'start': '16:00', 'end': '17:00', 'scheme': '方案5', 'cycle': 152},
        {'start': '17:00', 'end': '19:00', 'scheme': '方案17', 'cycle': 164},
        {'start': '19:00', 'end': '20:30', 'scheme': '方案7', 'cycle': 134},
        {'start': '20:30', 'end': '22:00', 'scheme': '方案8', 'cycle': 130},
        {'start': '22:00', 'end': '23:59', 'scheme': '方案9', 'cycle': 115},
    ],
...
def get_current_cycle_for_time(time_str):
    for scheme in CURRENT_SIGNAL_PLAN['现行周期方案']:
        ...
        if time_str >= start and time_str < end:
            return scheme['cycle'], scheme['scheme']
    return 99, '方案10'
...
def optimize_cycle_low_flow(phase_data, L_total, bounds):
    ...
    for phase, data in phase_data.items():
        if Y_total > 0:
            g_i = max(12.0, G * (data['y_val'] / Y_total))
        else:
            g_i = max(12.0, G / len(phase_data))
        
        lambda_i = g_i / best_C
        s_real_h = data['s_real_h']
        q_h = data['q_5min'] * 12
        c = s_real_h * lambda_i
        x = q_h / c if c > 0 else 0
        d_i = calculate_hcm_delay(best_C, lambda_i, x, c) if c > 0 else 9999.0
        
        best_allocation[phase] = {
            '有效绿灯(s)': round(g_i, 1),
    ...
    return best_C, 0, best_allocation
    ...
    for C in range(bounds['min'], bounds['max'] + 1):
        ...
        if surplus_g <= 0 or not coord_phases:
            ...
            for p, d in phase_data.items():
                temp_allocation[p] = max(12.0, G * (d['y_val'] / Y_total))
        else:
            y_coord_total = sum([d['y_val'] for d in coord_phases.values()])
            for p, d in coord_phases.items():
                if y_coord_total > 0:
                    g_actual = max(12.0, surplus_g * (d['y_val'] / y_coord_total))
                else:
                    g_actual = max(12.0, surplus_g / len(coord_phases))
                temp_allocation[p] = g_actual
...
def analyze_period(df, period):
    ...
    current_cycle, current_scheme = get_current_cycle_for_period(start, end)
    
    time_strs = get_time_strs_in_period(start, end)
    
    period_df = df[df['create_time'].dt.strftime('%H:%M').isin(time_strs)].copy()
    ...
    for phase_name, directions in CURRENT_SIGNAL_PLAN['专属相位映射'].items():
        ...
        for d, t in directions:
            ...
            if match:
                ...
                old_g = CURRENT_SIGNAL_PLAN['相位时间分配'].get(raw_name, 0) - CURRENT_SIGNAL_PLAN['相位损失时间'].get(raw_name, 0)
                if old_g > 0:
...
def run_period_analysis(df):
    ...
    for state, info in TRAFFIC_STATE_THRESHOLDS.items():
        ...
        if 'max_flow' in info:
            print(f"  {state}: ≤{info['max_flow']} pcu/5min | {desc}")
        elif 'min_flow' in info and 'max_flow' in info:
            print(f"  {state}: {info['min_flow']}-{info['max_flow']} pcu/5min | {desc}")
        else:
            print(f"  {state}: >{info['min_flow']} pcu/5min | {desc}")
