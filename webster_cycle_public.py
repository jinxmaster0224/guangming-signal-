...
CURRENT_SIGNAL_PLANS = {
    ...
    '光明大道与河心路': {
        '现行周期': 163,
        '放行相序': '东向全放 西向全放 -> 行人过街1 -> 东向全放 西向全放 -> 行人过街2 (二次过街)',
        '相位时间分配': {
            '东西同放1': 45,
            '行人过街1': 37,
            '东西同放2': 45,
        ...
        '专属相位映射': {
            # 相位1代表全部机动车需求 (Y)
            '相位1(东西同放1)': [('东向', '直行'), ('东向', '左转'), ('西向', '直行'), ('西向', '左转')],
            # 行人相位无机动车，列表置空，只消耗损失时间，不叠加 Y
            '相位2(行人过街1)': [], 
            '相位3(东西同放2_不计Y)': [], 
            '相位4(行人过街2)': []
        }
    ...
    '光明大道与华夏路': {
     ...
     '光明大道与华裕路': {
         '现行周期': 176,
         '放行相序': '东向全放 -> 东西直行同放 -> 西向全放 -> 南向全放 -> 北向全放',
         '相位时间分配': {
             '东向全放': 35,
             '东西直行同放': 29,
             '西向全放': 46,
         ...
         '专属相位映射': {
             # 防重复计算：该搭接相位中，东直在下一阶段继续亮绿，故此处仅提取被独占的左转作为需求驱动力
             '相位1(东向全放)': [('东向', '左转')],
...
def load_and_preprocess_data(csv_path):
    ...
    if 'create_time' in df.columns:
        ...
        df = df_target
        print(f"✅ 成功锁定目标日期：{TARGET_DATE}，共截取 {len(df)} 条快照数据。")
        ...
        raise ValueError("⚠️ 致命错误：CSV 中无法找到严格的时间列 (create_time 或 时间)！\n请确保输入的是保留了原始快照时间戳的底层明细表。")
    ...
    if '历史最大分均流量' in df.columns:
        ...
        print("✅ 成功直接加载 CSV 中的 [历史最大分均流量] 指标！")
    else:
        print("⚠️ 警告：CSV 中未找到 [历史最大分均流量] 列！")
        df['历史最大分均流量'] = 0
    ...
    for period_name, (start_time, end_time) in PEAK_PERIODS.items():
        ...
        for inter, group_inter in df_period.groupby('路口名称'):
            ...
            flow_stats = group_inter.groupby(['进口道方向', '转向']).agg(
                ...
                历史最大分均流量=('历史最大分均流量', 'first') # 直接拿现成的数据，绝不重算
            ).reset_index()
            
            total_intersection_flow = flow_stats['平均分均流量'].sum()
            if total_intersection_flow > 0:
                flow_stats['流量权重'] = flow_stats['平均分均流量'] / total_intersection_flow
            else:
                flow_stats['流量权重'] = 0.0
                
            flow_stats['加权延误'] = flow_stats['平均延误指数'] * flow_stats['流量权重']
            intersection_total_avg_delay = flow_stats['加权延误'].sum()
            
            inter_evals.append({
                '路口名称': inter,
                '交叉口总平均延误': intersection_total_avg_delay,
                '_flow_stats': flow_stats 
            })
            
        if inter_evals:
            df_eval = pd.DataFrame(inter_evals)
            df_eval = df_eval.sort_values(by=['交叉口总平均延误'], ascending=[False])
            
            critical_inter = df_eval.iloc[0]['路口名称']
            cici_score = df_eval.iloc[0]['交叉口总平均延误']
            critical_flow_stats = df_eval.iloc[0]['_flow_stats']
            
            print(f"【{period_name}】激活路口延误状况排行榜 (按流量加权总平均延误)：")
            display_df = df_eval[['路口名称', '交叉口总平均延误']].copy()
            display_df['交叉口总平均延误'] = display_df['交叉口总平均延误'].round(2)
            print(display_df.to_string(index=False))
            
            print(f"\n关键路口定位：")
            print(f"   -> 经过流量加权延误评估，当前研究范围关键路口为：🎯 {critical_inter}")
            print(f"      (入选理由：交叉口总平均延误最高，达 {cici_score:.2f})")
            ...
            if not cur_phase_mapping:
                print("⚠️ 警告：当前路口未配置 '专属相位映射'，将无法推演周期！")
                continue
