...
INTER_OFFSET_MAP = {
    '光明大道与光辉大道': 0.0, '光明大道与光明大街': -32.57, '光明大道与光安路': -33.69,
    '光明大道与华夏路': -63.5, '光明大道与华裕路': -63.0,
}
...
def analyze_multiple_files(file_paths, output_dir):
    ...
    if global_granular_data:
        ...
        out_path = os.path.join(output_dir, "干线全流向_延误与流量占比明细表.csv")
...
def run_tod_partition(file_paths, output_dir):
    ...
    # 【更新】采用字典分别存储双轨划分结果，并构建统一csv输出格式
    ...
    out_path = os.path.join(output_dir, '时段划分结果表.csv')
...
def run_period_analysis(df_path, time_periods, output_dir, current_plan, day_type, target_date):
    ...
    out_filename = f"各时段周期优化结果_光明大街{day_type}.csv"
...
if __name__ == "__main__":
    ...
    generated_files = []  # 用于记录生成的文件信息

    if not csv_files:
        print("❌ 未在当前目录下找到 'index_view' 数据文件！")
    else:
        # 【新增】根据路口名称动态创建输出专属文件夹
        target_dir = os.path.join(script_dir, f"{INTERSECTION}信控方案")
        os.makedirs(target_dir, exist_ok=True)
        
        # Phase 1: 数据清洗与特征提取 (输出至 target_dir)
        processed_data_path = analyze_multiple_files(csv_files, target_dir)
        
        if processed_data_path:
            generated_files.append(("底层聚合清洗数据明细表", "干线全流向_延误与流量占比明细表.csv", processed_data_path))
            
            # Phase 2: Fisher时段划分提取 (输出至 target_dir)
            ...
            if dynamic_periods_dict[DAY_TYPE_WD] and dynamic_periods_dict[DAY_TYPE_WE]:
                generated_files.append(("Fisher肘部时段划分结果", "时段划分结果表.csv", periods_path))
                
                # Phase 3-A: 工作日周期绿信比推演 (输出至 target_dir)
                ...
                # Phase 3-B: 非工作日周期绿信比推演 (输出至 target_dir)
