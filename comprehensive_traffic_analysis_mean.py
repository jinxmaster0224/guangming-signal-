...
def generate_turning_flow_charts(df, intersection_name, output_dir, interval):
    ...
    try:
        ...
        fig_sub.suptitle(f'{intersection_name} - 各进口道转向分均车流量 ({interval}分钟聚合)', fontsize=18, y=0.98)
        ...
        fig_all.suptitle(f'{intersection_name} - 全进口道转向分均流量对比 ({interval}分钟聚合)', fontsize=18)
...
def generate_charts(df, intersection_name, output_dir, interval, df_dir=None):
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f'queue_len_max_trend_{interval}min.png'))
        plt.close()
        ...
        ax.legend(loc='upper left')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f'queue_len_avg_trend_{interval}min.png'))
        plt.close()
        ...
        for i, date in enumerate(dates):
            ...
            if 'pass_flow' in date_df.columns:
                ax.plot(normalized_time, date_df['pass_flow'], marker='o', markersize=3, linewidth=1, 
                         color=colors[i % len(colors)], label=str(date))
        ax.set_title(f'{intersection_name} - 分均车流量变化趋势 ({interval}分钟)')
        ax.set_xlabel('时间')
        ...
        ax.legend(loc='upper left')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f'pass_flow_trend_{interval}min.png'))
        plt.close()
        ...
        ax.legend(loc='upper left')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f'stop_time_trend_{interval}min.png'))
        plt.close()
        ...
        ax.legend(loc='upper left')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f'stop_times_trend_{interval}min.png'))
        plt.close()
        ...
        ax.legend(loc='upper left')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f'pass_speed_trend_{interval}min.png'))
        plt.close()
        ...
        ax.legend(loc='upper left')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f'no_stop_pass_speed_trend_{interval}min.png'))
        plt.close()
        ...
        ax.legend(loc='upper left')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f'delay_index_trend_{interval}min.png'))
        plt.close()
        ...
        ax.legend(loc='upper left')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f'idx_state_trend_{interval}min.png'))
        plt.close()
        ...
        if 'los' in df.columns:
            ...
            ax.legend(loc='upper left')
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, f'los_trend_{interval}min.png'))
            plt.close()
        ...
        if df_dir is not None and not df_dir.empty and 'turn_dir_no' in df_dir.columns and 'main_direction' in df_dir.columns:
            ...
            # 💡 防错机制：如果没有读到副表，在这个格子里用文字提示，证明代码已生效！
...
def generate_vertical_comparison_charts(data_by_interval, intersection_name, output_dir):
    ...
    try:
        ...
        for metric in metrics:
            ...
            plt.tight_layout(rect=[0, 0, 1, 0.96])
            plt.savefig(os.path.join(comparison_dir, f'{metric["name"]}_时间间隔对比.png'))
            plt.close()
        ...
        if 'los' in list(data_by_interval.values())[0].columns:
            ...
            plt.tight_layout(rect=[0, 0, 1, 0.96])
            plt.savefig(os.path.join(comparison_dir, '延误指数评级_时间间隔对比.png'))
            plt.close()
