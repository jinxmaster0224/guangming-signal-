...
def estimate_subzone_ccl(df):
    print("\n" + "="*80)
    print(f" 🚦 启动子区公共周期 (CCL) - 修正 Webster 精算引擎 (日期: {TARGET_DATE})")
    print("="*80)
    ...
    for period_name, (start_time, end_time) in PEAK_PERIODS.items():
        ...
        if inter_evals:
            ...
            print(f"      (入选理由：全线综合拥堵指数最高达 {cici_score:.2f})")
                ...
                for d, t in directions:
                    ...
                    if not mov.empty:
                        ...
                        if max_flow > 0:
                            ...
                            detail_str = f"⚠️绝量缺失, 按占比{ratio:.1f}%折算 = y:{y_val:.3f}"
            ...
            print(f"   [精算结果]：({alp} × {L_total} + {bet}) / (1 - {the} × {Y_val:.3f}) ≈ {subzone_ccl}s")
