...
STATE_OPTIMIZATION_WEIGHTS = {
    '极低流量': {
        'delay_weight': 0.6,       # 权重侧重于降低整体延误
        'min_green_weight': 0.4    # 侧重于保障最小安全绿灯时间
    },
    '平峰流量': {
        'delay_weight': 0.4,      
...
def optimize_cycle_low_flow(phase_data, L_total, bounds):
    ...
    # 按照你的要求，开始遍历极低流量的周期范围 (例如 90s 到 110s)
    for C in range(bounds['min'], bounds['max'] + 1):
        ...
        total_delay = 0.0
        min_green_ratio = float('inf')
        min_green_seconds = float('inf')
        valid_cycle = True
        
        # 依然坚持最合理的等饱和度(y值比例)分配法
        for phase, data in phase_data.items():
            if Y_total > 0:
                g_i = G * (data['y_val'] / Y_total)
            else:
                g_i = G / len(phase_data)
            
            lambda_i = g_i / C
            
            # 记录本周期内最短的绿灯时间和最小的绿信比，用于后续打分惩罚
            min_green_ratio = min(min_green_ratio, lambda_i)
            min_green_seconds = min(min_green_seconds, g_i)
        ...
        # ==========================================
        # 核心修改：仅根据【延误状况】和【最小绿灯满足度】计算得分
        # ==========================================
        # 1. 延误得分 (基础惩罚，延误越大得分越高)
        delay_score = total_delay / 100
        
        # 2. 最小绿灯满足度得分 (软约束惩罚)
        # 设定底线：绿灯不能低于12秒，绿信比不能低于0.12。一旦低于这个值，施加高额惩罚分。
        min_green_score = max(0, 12.0 - min_green_seconds) * 2 + max(0, 0.12 - min_green_ratio) * 10
        
        # 计算综合目标值 (取消了与 bounds['default'] 的偏离度惩罚)
        objective = (
            weights.get('delay_weight', 0) * delay_score +
            weights.get('min_green_weight', 0) * min_green_score
        )
    ...
    # 极端数据保护：如果遍历一圈没有得到有效方案，默认使用平峰逻辑兜底
...
def load_and_preprocess_data(csv_path):
    ...
    # ==========================
    # 【新增逻辑：修正数据延迟与错位】
    # 规则：00:10 的批次代表 00:00-00:05 的真实路况。
    # 我们将时间戳对齐到业务时段起点，即往前推 10 分钟。
    # ==========================
    df['true_start_time'] = df['create_time'] - pd.Timedelta(minutes=10)
    
    # 过滤日期时，必须用 true_start_time，否则跨天的凌晨数据会被错误切断
...
def analyze_period(df, period):
    ...
    # ==========================
    # 【修改逻辑：利用 true_start_time 提取，并自然消除滑动重叠】
    # ==========================
