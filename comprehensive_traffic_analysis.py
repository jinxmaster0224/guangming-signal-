...
def analyze_traffic_data(dir_csv_path, info_csv_path, date_ranges, offset_degree=0.0):
    ...
    if info_csv_path and os.path.exists(info_csv_path):
        # 【关键修复】：在这里加入了 'inter_name'，让它从 info 表里读取名字
        ...
        if not df_info.empty:
            ...
            # 严格去重，避免 left join 导致数据量爆炸
            df_info = df_info.drop_duplicates(subset=['create_time'])
            
            # 合并前必须把 inter_name 删掉，否则合并后会出现 inter_name_x 和 inter_name_y 导致冲突
    ...
    # 【彻底解决标题问题】：如果有名字，就不加多余符号；没名字就完全留白，不显示 Target_Intersection
...
if __name__ == "__main__":
    ...
    # 如果没找到标准命名，回退寻找包含 ods_gaode 的普通文件
    if not dir_csv:
