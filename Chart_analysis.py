...
def prepare_data(df, inter_id):
    ...
    # 对速度数据进行逻辑校验：不停车通过速度 不应低于 实际平均通过速度
    if 'no_stop_pass_speed' in df_inter.columns and 'pass_speed' in df_inter.columns:
        df_inter['no_stop_pass_speed'] = np.maximum(df_inter['no_stop_pass_speed'], df_inter['pass_speed'])
    
    # 剔除缺失核心指标的空行 (增加对两个速度字段的非空校验)
    df_inter = df_inter.dropna(subset=['pass_flow', 'queue_len_max', 'stop_time', 'pass_speed', 'no_stop_pass_speed'])
    ...
    if not df_agg.empty:
        total_data = df_agg.groupby('create_time').apply(
            lambda x: pd.Series({
                ...
                'pass_speed': np.average(x['pass_speed'], weights=x['pass_flow'] + 1e-5),
                'no_stop_pass_speed': np.average(x['no_stop_pass_speed'], weights=x['pass_flow'] + 1e-5),
...
def plot_9_grid(data_dict, x_col, y_col, title, x_label, y_label, save_path, color_col=None, cbar_label='延误指数'):
    """
    绘制 3x3 九宫格散点图并保存到本地
    """
    if not data_dict:
...
def plot_single(df, x_col, y_col, title, x_label, y_label, save_path, color_col=None, cbar_label='延误指数'):
    """
    单独绘制一张大图并保存到本地
    """
    ...
    if not csv_files:
        ...
        try:
            ...
            if target_inter_id not in df['inter_id'].unique():
                ...
                # 1. 流率-排队图（新增颜色渐变与自定义标签）
                plot_9_grid(
                    data_dict=plot_data, 
                    x_col='queue_len_max', 
                    y_col='pass_flow', 
                    title=f'{inter_name} 流量-最大排队长度图 (各转向)',
                    x_label='最大排队长度 (queue_len_max) [米]',
                    y_label='5min车流量 (pass_flow) [辆]',
                    save_path=os.path.join(output_folder, f"{inter_name}_九宫格_流量_最大排队长度.png"),
                    color_col='queue_len_max',  # 用排队长度做渐变色
                    cbar_label='严重程度'         # 修改颜色条的标题
                )
                ...
                plot_9_grid(
                    ...
                    x_col='stop_time', 
                    y_col='pass_flow', 
                    title=f'{inter_name} 流量-停车时间图 (各转向)',
                    x_label='车均停车时间 (stop_time) [秒]',
                    y_label='5min车流量 (pass_flow) [辆]',
                    save_path=os.path.join(output_folder, f"{inter_name}_九宫格_流量_停车时间.png"),
                ...
                plot_9_grid(
                    ...
                    x_col='pass_speed', 
                    y_col='pass_flow', 
                    title=f'{inter_name} 流量-通过速度图 (各转向)',
                    x_label='平均通过速度 (pass_speed) [km/h]',
                    y_label='5min车流量 (pass_flow) [辆]',
                    save_path=os.path.join(output_folder, f"{inter_name}_九宫格_流量_通过速度.png"),
                    color_col='delay_index' 
                )
                ...
                plot_9_grid(
                    data_dict=plot_data, 
                    x_col='no_stop_pass_speed', 
                    y_col='pass_flow', 
                    title=f'{inter_name} 流量-不停车速度图 (各转向)',
                    x_label='不停车通过速度 (no_stop_pass_speed) [km/h]',
                    y_label='5min车流量 (pass_flow) [辆]',
                    save_path=os.path.join(output_folder, f"{inter_name}_九宫格_流量_不停车速度.png"),
                    color_col='delay_index' 
                )
                ...
                if total_df is not None and not total_df.empty:
                    ...
                    plot_single(
                        ...
                        x_col='queue_len_max', 
                        y_col='pass_flow', 
                        title=f'{inter_name} 流量-最大排队长度图',
                        x_label='最大排队长度 (queue_len_max) [米]',
                        y_label='5min车流量 (pass_flow) [辆]',
                        save_path=os.path.join(output_folder, f"{inter_name}_独立总图_流量-最大排队长度.png"),
                        color_col='queue_len_max',  # 用排队长度做渐变色
                        cbar_label='严重程度'         # 修改颜色条的标题
                    )
                    ...
                    plot_single(
                        ...
                        x_col='stop_time', 
                        y_col='pass_flow', 
                        title=f'{inter_name} 流量-停车时间图',
                        x_label='车均停车时间 (stop_time) [秒]',
                        y_label='5min车流量 (pass_flow) [辆]',
                        save_path=os.path.join(output_folder, f"{inter_name}_独立总图_流量-停车时间.png"),
                        color_col='delay_index'
                    )
                    ...
                    plot_single(
                        ...
                        x_col='pass_speed', 
                        y_col='pass_flow', 
                        title=f'{inter_name} 流量-通过速度图',
                        x_label='通过速度 (pass_speed) [m/s]',
                        y_label='5min车流量 (pass_flow) [辆]',
                        save_path=os.path.join(output_folder, f"{inter_name}_独立总图_流量-通过速度.png"),
                        color_col='delay_index'
                    )
                    ...
                    plot_single(
                        ...
                        x_col='no_stop_pass_speed', 
                        y_col='pass_flow', 
                        title=f'{inter_name} 流量-不停车通过速度图',
                        x_label='不停车通过速度 (no_stop_pass_speed) [m/s]',
                        y_label='5min车流量 (pass_flow) [辆]',
                        save_path=os.path.join(output_folder, f"{inter_name}_独立总图_流量-不停车通过速度.png"),
                        color_col='delay_index'
                    )
