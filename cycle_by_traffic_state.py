...
def optimize_cycle_low_flow(phase_data, L_total, bounds):
    ...
    for C in range(bounds['min'], bounds['max'] + 1):
        ...
        for phase, data in phase_data.items():
            if Y_total > 0:
                temp_allocation[phase] = base_g + surplus_g * (data['y_val'] / Y_total)
            else:
                temp_allocation[phase] = base_g + surplus_g / num_phases
                
        # =========================================================
        # 最大余额法：严谨分配整数绿灯，确保总和精确等于 G
        int_g = {p: int(math.floor(g)) for p, g in temp_allocation.items()}
        remainders = {p: g - int_g[p] for p, g in temp_allocation.items()}
        missing_seconds = int(round(G - sum(int_g.values())))
        sorted_phases = sorted(remainders.keys(), key=lambda p: remainders[p], reverse=True)
        for i in range(missing_seconds):
            int_g[sorted_phases[i]] += 1
        temp_allocation = int_g
        # =========================================================
...
def optimize_cycle_peak(phase_data, L_total, bounds):
    ...
    for C in range(bounds['min'], bounds['max'] + 1):
        ...
        for p, d in phase_data.items():
            if Y_total > 0:
                extra_g = surplus_g * (d['y_val'] / Y_total)
            else:
                extra_g = surplus_g / num_phases
                
            temp_allocation[p] = base_g + extra_g
...
if __name__ == "__main__":
    ...
    try:
        ...
        output_path = os.path.join(script_dir, "各时段周期优化结果_光明大街非工作日.csv")
